...
{
    public class NodeTableCell : AdfChildNode
    {
        ...
        public NodeTableCell(List<AdfTopNode> content) : base(NodeTypeChild.tableCell)
        {
            Content = content;
        }

        public NodeTableCell(List<AdfTopNode> content, string? backgroundColor, List<int>? colwidth, int colspan = 1, int rowspan = 1) : this(content)
        {
        ...
        private void ValidateContent(List<AdfTopNode> content)
        {
            ...
            {
                throw new ArgumentException("NodeTableCell: Content must contain 1 or more elements");
            }
            ...
            if (!flag)
            {
                throw new ArgumentException("NodeTableCell: Must be a blockquote, bullet list, ordered list, code block, media group, heading, panel, paragraph, or rule");
            }
        }
    }
}
